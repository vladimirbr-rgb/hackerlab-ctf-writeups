...
Цель задания — проанализировать предоставленный архив с Python-скриптом и набором клиентских файлов, определить механизм преобразования данных и восстановить результат, необходимый для завершения задания на HackerLab.
...
Архив задания содержал Python-скрипт обработки данных и клиентские файлы. В ходе работы были выполнены следующие действия:

1. Подготовлена Ubuntu VM и проверено рабочее окружение.
2. Обновлён список пакетов и установлен `unzip`.
3. Распакован архив `anonymizator.zip`.
4. Выполнена инвентаризация доступных файлов.
5. Изучена логика преобразования данных в Python-скрипте.
6. Подготовлен и проверен Python-декодер `decode.py`.
7. Результат был проверен и отправлен на платформу HackerLab.
...
> Команда декодирования и её результат не публикуются, чтобы не раскрывать флаг, персональные данные из учебных файлов и другие чувствительные материалы.
...
Задание показало практический процесс анализа Python-скрипта и восстановления обработанных данных: от подготовки Linux-окружения и распаковки артефактов до проверки логики декодирования.
